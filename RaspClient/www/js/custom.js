...
function charts() {
	...
	if($("#temperature").length)
	{
		...
		var a = [19,19,18,17,17,18,20,21,21,21,22,24,
						25,29,27,26,25,25,23,22,22,21,21,20];
		...
		$("#temperature").bind("plotclick", function (event, pos, item) {
			if (item) {
				$("#clickdata").text("You clicked point " + item.dataIndex + " in " + item.series.label + ".");
				plot.highlight(item.series, item.datapoint);
			}
	...
	if($("#humidity").length)
	{
		...
		var b = [40,41,44,50,50,60,51,55,56,60,70,51,
						50,60,60,67,65,66,60,45,44,44,43,42];

		for (var i = 0; i < 23; i += 1) {
			h.push([i, b[i]]);
		}

		var plot = $.plot($("#humidity"),
				[ { data: h, label: "湿度" } ], {
					series: {
						lines: { show: true,
								lineWidth: 2,
		...
		$("#humidity").bind("plotclick", function (event, pos, item) {
			if (item) {
				$("#clickdata").text("You clicked point " + item.dataIndex + " in " + item.series.label + ".");
				plot.highlight(item.series, item.datapoint);
			}
